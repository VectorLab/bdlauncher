...
template <size_t size> struct StrictMem {
  ...
  template <size_t use>
  INLINE auto operator<<(char const (&data)[use]) -> StrictMem<size - use + 1> {
    static_assert(size >= use - 1);
    memcpy(current, data, use - 1);
    return {current + use - 1};
  }
  template <typename T>
  INLINE auto operator<<(T addr)
      -> std::enable_if_t<std::is_same_v<T, void *>,
                          StrictMem<size - sizeof(void *)>> {
...
template <size_t base> class AutoGrowMem {
  ...
  INLINE void realloc() {
    current =
        static_cast<char *>(mmap(0, base, PROT_READ | PROT_WRITE | PROT_EXEC,
...
char *wr_regcmd(void *call, void *arg) {
 ...
 0x0000000000001191 <+1>:	48 b8 77 77 77 77 ff ff ff ff	movabs
 rax,0xffffffff77777777 0x000000000000119b <+11>:	48 bf 77 77 77 77 ff ff
 ff ff	movabs rdi,0xffffffff77777777 0x00000000000011a5 <+21>:	ff d0	call rax
 ...
 */
  ...
  mem << "\x57\x48\xb8" << call << "\x48\xbf" << arg
      << "\xff\xd0\x5f\x48\x89\x07\x48\x89\xf8\xc3";
  return mem;
};
