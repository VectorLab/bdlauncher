...
#define HOOK(type, method)                                                     \
  MyHook(void_cast(&type ::method), void_cast(&type ::CONS($$, method)),       \
         (void **)&type ::CONS($, method)())
...
#define _TStaticHook(pclass, iname, sym, ret, args...)                         \
  ...
  static RegisterStaticHook _TRStaticHook_##iname(                             \
      #sym, &_TStaticHook_##iname::_hook,                                      \
      (void **)&_TStaticHook_##iname::_original);                              \
  ret (*_TStaticHook_##iname::_original)(args);                                \
  ret _TStaticHook_##iname::_hook(args)
#define _TStaticDefHook(iname, sym, ret, type, args...)                        \
  _TStaticHook( : public type, iname, sym, ret, args)
#define _TStaticNoDefHook(iname, sym, ret, args...)                            \
  _TStaticHook(, iname, sym, ret, args)

#define THook2(iname, ret, sym, args...)                                       \
  _TStaticNoDefHook(iname, sym, ret, args)
#define THook(ret, sym, args...) THook2(sym, ret, sym, args)
#define TStaticHook2(iname, ret, sym, type, args...)                           \
  _TStaticDefHook(iname, sym, ret, type, args)
#define TStaticHook(ret, sym, type, args...)                                   \
  TStaticHook2(sym, ret, sym, type, args)
