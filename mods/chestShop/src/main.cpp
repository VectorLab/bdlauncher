...
struct Chest {
  ...
  bool do_buy(const string &ply, int slot) {
    if (slot >= 27)
      return false;
    if (slots[slot].price == 0)
      return false;
    int price = slots[slot].price;
    if (money_red(ply, price)) {
      money_add(slots[slot].owner, price);
      slots[slot].price = 0;
...
static void r_save() {
  ...
  ofstream ff{configpath.c_str(), ios::trunc | ios::out};
  ff << data;
  ff.close();
}
...
static void load() {
  ...
  if (stat(configpath.c_str(), &buf) != 0) {
    int fd = open(configpath.c_str(), O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
  ...
  for (auto &i : value) {
    ...
    int ct = 0;
    for (auto &j : i["slots"])
      ch.slots[ct++] = Slot(j[0].asString(""), j[1].asInt(0));
    lc.emplace_back(ch);
  }
}
...
THook(void *, _ZN15ChestBlockActor8stopOpenER6Player, BlockActor &ac,
      Player &pl) {
...
THook(void *, _ZN15ChestBlockActor9startOpenER6Player, BlockActor &ac,
      Player &pl) {
  auto &pos = ac.getPosition();
  // do_log("start pos %d %d %d",pos.x,pos.y,pos.z);
  Chest *gch = fetchChest(pos.x, pos.y, pos.z);
  if (gch) {
    sessions.emplace(pl.getNameTag(), gch);
  }
  return original(ac, pl);
}
THook(unsigned long, _ZNK20InventoryTransaction11executeFullER6Playerb,
      void *_thi, Player &player, bool b) {
  ...
  if (pullcnt) {
    ...
    char buf[1000];
    sprintf(buf, "item %s by %s price %d,use /cshop buy to buy it",
            pullstr.c_str(), x.owner.c_str(), x.price);
    sendText(&player, buf);
...
static void oncmd(argVec &a, CommandOrigin const &b, CommandOutput &outp) {
  ...
  auto name = b.getName();
  if (a[0] == "buy") {
    if (!ply_select.count(name) || sessions.count(name) ||
        !last_sess.count(name)) {
...
static bool handle_u(GameMode *a0, ItemStack *a1, BlockPos const *a2,
