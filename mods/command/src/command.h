...
protected:
   ...
   */
  CustomCommandContext(CommandOrigin const &origin,
                       CommandOutput &output) noexcept
...
template <typename DynEnum> class CustomDynEnumHandle {
  ...
  CustomDynEnumHandle(CustomCommandRegistry *registry) noexcept
      : registry(registry) {}
...
class CustomCommandRegistry {
  ...
  template <typename Return, typename... Params> struct ApplicationArray {
    ...
    template <typename Target, typename... ConsParams>
    inline Target &append(ConsParams... cons) {
...
public:
  ...
  class EnumApplication : public Application<void, ::CommandRegistry *> {
    friend class CustomCommandRegistry;
  ...
  public:
     ...
     * @param value value
     */
    void addValue(std::string_view name, Enum value) noexcept {
  ...
  class CustomDynEnumApplication
      : public Application<void, ::CommandRegistry *> {
    static_assert(std::is_base_of_v<CustomDynEnum<DynEnum>, DynEnum>);
    template <typename Return, typename... Params>
    friend class ApplicationArray;
    ...
    CustomDynEnumApplication(std::initializer_list<std::string> values)
        : values(values) {}
  };
  ...
  class CommandApplication : public Application<void, ::CommandRegistry *> {
  ...
  public:
    ...
    class OverloadApplication
        : public Application<::CommandRegistry::Overload> {
    ...
    public:
      ...
      struct ParameterApplication : Application<::CommandParameterData> {
        char const *name;
        bool optional;
        size_t offset;
       ...
       */
      template <typename Type>
      void addParameter(char const *name, bool optional, size_t offset) {
        param_applications.append<ParameterApplication<Type>>(name, optional,
                                                              offset);
      }
    };
     ...
     * @return OverloadApplication<Host> Command overload registering
     * application
     */
...
inline void CustomCommandRegistry::startRegister(::CommandRegistry *registry) {
  for (auto &application : applications)
    application->apply(registry);
  applications.clear();
}

template <typename DynEnum>
void CustomCommandRegistry::CustomDynEnumApplication<DynEnum>::apply(
...
template <typename Enum>
void CustomCommandRegistry::EnumApplication<Enum>::apply(
    ::CommandRegistry *registry) {
...
template <typename Desc>
void CustomCommandRegistry::CommandApplication<Desc>::apply(
    ::CommandRegistry *registry) {
  std::string name = Desc::cmd_name;
  do_log("register command %s", name.c_str());
  registry->registerCommand(name, Desc::description, Desc::permission,
  ...
  for (auto &overload : overload_applications) {
    ...
    registry->registerOverloadInternal(*signature,
                                       *signature->overloads.rbegin());
  }
...
template <typename Desc>
template <typename Host>
::CommandRegistry::Overload CustomCommandRegistry::CommandApplication<
    Desc>::OverloadApplication<Host>::apply() {
  ::CommandRegistry::Overload ret{
      {0, std::numeric_limits<int>::max()},
      +[]() -> std::unique_ptr<::Command> { return std::make_unique<Host>(); },
  };
  for (auto &param : param_applications)
    ret.params.emplace_back(param->apply());
...
template <typename Host>
template <typename Type>
::CommandParameterData CustomCommandRegistry::CommandApplication<
    Desc>::OverloadApplication<Host>::ParameterApplication<Type>::apply() {
  if constexpr (CommandParameterProxy<Type>::type ==
