...
THook(
    void *,
    _ZN20ServerNetworkHandler6handleERK17NetworkIdentifierRK23ModalFormResponsePacket,
  ...
  if (p) {
    ...
    if (it != player_mp.dat.end()) {
      ...
      if (id == form->fid) {
        ...
        form->needfree = false;
        player_mp.dat.erase(
            p); // WHY USE THIS SHIT HACK? A FORM CAN BE OPENED IN ANOTHER ONE
        // printf("recv %d %d\n",id,form->fid);
...
void gui_ChoosePlayer(
    ServerPlayer *sp, string_view text, string_view title,
