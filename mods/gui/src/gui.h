...
constexpr std::string_view input_json =
...
struct SharedForm {
  ...
  SharedForm(string_view title, string_view cont, bool nedfree = true,
             bool isInp = false) {
    needfree = nedfree;
    isInput = isInp;
    hasSerialed = false;
  ...
  void addInput(string_view text) {
    assert(isInput);
    buf.write("{\"type\":\"input\",\"placeholder\":null,\"default\":null,"
              "\"text\":\"");
    buf.write(text);
...
PUB_EXPORT void sendForm(ServerPlayer &sp, SharedForm *fm);
PUB_EXPORT SharedForm *getForm(string_view title, string_view cont,
                               bool isInp = false);
PUB_EXPORT void
gui_ChoosePlayer(ServerPlayer *sp, string_view text, string_view title,
                 std::function<void(ServerPlayer *, string_view)> const &cb);
