...
#include <minecraft/actor/PlayerInventory.h>
...
ItemStack *createItemStack_static(string const &name, unsigned char amo,
                                  ItemStack *stk) {
  char filler[16];
  memset(filler, 0, sizeof(filler));
  int unk = 0;
  _ZN12ItemRegistry12lookupByNameERiRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEE(
      filler, unk, &name);
  Item *ite = nullptr;
...
ItemStack *createItemStack_static(short id, short aux, unsigned char amo,
                                  ItemStack *stk) {
  char filler[16];
  memset(filler, 0, sizeof(filler));
  // int unk = 0;
...
bool takeItem(ServerPlayer &sp, ItemStack *is) {
  ...
  int sz = x->getContainerSize((ContainerID)0);
  int sigmasz = 0;
  short id = is->getId();
  short aux = is->getAuxValue();
  for (int i = 0; i < sz; ++i) {
    auto &item = x->getItem(i, (ContainerID)0);
