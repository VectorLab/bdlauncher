...
THook(void *, _ZN12ServerPlayer27setLocalPlayerAsInitializedEv,
      ServerPlayer *sp) {
...
THook(void *, _ZN20ServerNetworkHandler13_onPlayerLeftEP12ServerPlayerb,
      void *a, ServerPlayer *sp, bool x) {
...
THook(void *, _ZN8GameMode12destroyBlockERK8BlockPosh, GameMode *sp,
      BlockPos const &x, unsigned char cc) {
  int fg = 1;
...
static_deque<bool (*)(GameMode *a0, ItemStack *a1, BlockPos const *a2,
                      BlockPos const *dstPos, Block const *a5)>
    useion_hook;
THook(void *, _ZN8GameMode9useItemOnER9ItemStackRK8BlockPoshRK4Vec3PK5Block,
      GameMode *sp, ItemStack &ite, BlockPos const &bpos, unsigned char side,
      Vec3 const &unk2, Block const *bl) {
  int fg = 1;
...
THook(
    void *,
    _ZN20ServerNetworkHandler6handleERK17NetworkIdentifierRK23ItemFrameDropItemPacket,
...
THook(void *, _ZN3Mob3dieERK17ActorDamageSource, Mob &th,
      ActorDamageSource const &src) {
...
THook(void *, _ZN3Mob5_hurtERK17ActorDamageSourceibb, Mob &th,
      ActorDamageSource const &src, int val, bool unk, bool unk2) {
  int nprevent = 1;
  // do_log("call %ld %ld
  // %d",th.getUniqueID().id,src.getEntityUniqueID().id,val);
  for (auto &i : mobhurt_hook) {
    nprevent &= i(th, src, val);
  }
  return nprevent ? original(th, src, val, unk, unk2) : nullptr;
}
static_deque<bool (*)(Actor &, ActorDamageSource const &, int &)>
    actorhurt_hook;

THook(void *, _ZN5Actor4hurtERK17ActorDamageSourceibb, Actor &th,
      ActorDamageSource const &src, int val, bool unk, bool unk2) {
  int nprevent = 1;
...
THook(void *, _ZN8GameMode8interactER5ActorRK4Vec3, GameMode *gm, Actor &ac,
      Vec3 const &vc) {
  int nprevent = 1;
...
#define regAPI(name, hk)                                                       \
  void reg_##name(decltype(&(***hk##_hook.begin())) x) {                       \
    hk##_hook.push_back(x);                                                    \
  }
