...
THook(void *, _ZN14ServerInstance14onLevelCorruptEv, void *x) {
  ...
  for (auto &p : *vc) {
    auto sp = getSP(p.get());
    if (sp) {
      payload += sp->getNameTag() + " ->(" + std::to_string(sp->getPos().x) +
                 " , " + std::to_string(sp->getPos().z) + ") dim " +
                 std::to_string(sp->getDimensionId()) + "\n";
    }
  }
  printf("%s\n", payload.c_str());
  char buf[1024];
...
THook(
    void *,
    _ZN12ServerPlayerC2ER5LevelR12PacketSenderR14NetworkHandlerRN15ClientBlobCache6Server22ActiveTransfersManagerE8GameTypeRK17NetworkIdentifierhSt8functionIFvRS_EEN3mce4UUIDERKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEESt10unique_ptrI11CertificateSt14default_deleteIST_EEib,
    ServerPlayer *a1, Level *a2, void *a3, void *a4, void *a5, void *a6,
    NetworkIdentifier *a7, unsigned char a8, void *a9, void *a10, void *a10_1,
    string *a11, Certificate *a12, void *a13) {
  auto ret =
      original(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a10_1, a11, a12, a13);
  if (SPEC_NETI_OFFSET == 0) {
    // seek for neti offset
    for (int i = 2000; i < 4000; ++i) {
      if (memcmp(((char *)a1) + i, a7, 0x98) == 0) {
        if (SPEC_NETI_OFFSET) {
          do_log("wtf!duplicate network iden??? first:%ld next %d",
                 SPEC_NETI_OFFSET, i);
        }
        SPEC_NETI_OFFSET = i;
      }
...
THook(
    ...
    std::__cxx11::basic_string<char, std::char_traits<char>,
                               std::allocator<char>> const &d,
    std::__cxx11::basic_string<char, std::char_traits<char>,
                               std::allocator<char>> const &b,
    PermissionsFile *c) {
...
THook(
    void *,
    _ZN15DedicatedServer5startERKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEE,
    DedicatedServer *t, string &b) {
...
void onLoad(ModContext *__tmp_1) {
  ...
  do_log("fake vtable pt %p %p %p %p", (void *)pt[0], (void *)pt[1],
         (void *)pt[2],
         _ZN19ServerCommandOriginD2Ev); // pt[3]=D0ev
  fake_vtbl[2] = fake_vtbl[3] =
      (void *)dummy__; // fix free(ServerCommandOrigin)
  memcpy(fake_vtbl_ply, (void *)(void *)_ZTV19PlayerCommandOrigin,
         sizeof(fake_vtbl_ply));
